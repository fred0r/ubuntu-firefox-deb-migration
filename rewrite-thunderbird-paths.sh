...
Old roots rewritten:
  ...
  <macOS>/Users/<user>/Library/Thunderbird   (detected automatically)
...
pattern = re.compile(r"/Users/[^/]+/Library/Thunderbird")

seen = set()
for root, dirs, files in os.walk(profile_dir):
    dirs[:] = [d for d in dirs if d not in ("cache2", "startupCache", "ImapMail")]
    for name in files:
...
fix_sqlite() {
  ...
  python3 - "$file" "$HOME/snap/thunderbird/common/.thunderbird" "$HOME/.var/app/org.mozilla.Thunderbird/.thunderbird" "$HOME/.thunderbird" "$OLD_MAC_ROOT" "$dry" <<'PY_SQLITE'
import sqlite3
import sys
...
main() {
  ...
  local -a ini_patterns=(-e "$HOME/snap/thunderbird/common/.thunderbird" -e "$HOME/.var/app/org.mozilla.Thunderbird/.thunderbird")
  if [[ -n "$OLD_MAC_ROOT" ]]; then
