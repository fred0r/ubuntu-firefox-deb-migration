...
Window-state files (sessionstore.jsonlz4, xulstore.json) are reset so stale or
corrupt state cannot break the launch after migration; Firefox regenerates them.
...
reset_window_state() {
  local profile_dir="$1"
  local f
  for f in sessionstore.jsonlz4 xulstore.json; do
